<script lang="ts">
  import { deleteDevice } from "$lib/api/Devices.svelte";
  ...
  <div class="bg-neutral-900/75 rounded-xl py-3 px-6 shadow-lg w-full">
    <div class="flex justify-between items-start gap-2">
      ...
      <div class="flex gap-2">
        ...
        <button aria-label="Delete" class="p-2 rounded-md text-red-500 hover:bg-red-500/10 transition cursor-pointer" on:click={() => deleteDeviceHandler(id)}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-5">
